...
char *tok;
//check whether a string contain a certain word such as string
...
void setup(char in_buffer[], char *args[],int *background)
{
	...
	length = read(STDIN_FILENO, in_buffer, MAXLINE);
...
{
	system("clear");
	char in_buffer[MAXLINE]; /* to store the input commands*/
	int background;             /*is set if '&' is given at end of command */
	char *args[MAXLINE/2+1];/* to have 40 arguments */
	...
	{           
		...
		if(checkCommand[0] == '!' )
		{
					...
					&& position_value <= maxIndex))
			{
				...
				*/
				...
				/**
					* This for loop will parse the temporary array
		...
		*/
		...
		//This block is to execute cd part;
		bzero(buffer, BUFFER_SIZE);
        fgets(buffer, BUFFER_SIZE, stdin);
        if(hasPrefix(buffer,";") == 0)
        {
        	tok = strchr(buffer,';'); 
        	system(tok);
        }
